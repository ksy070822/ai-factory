...
function convertToGroupPredictions(apiData: any[]): GroupPrediction[] {
  return apiData.map((data) => {
    ...
    const watchReasons = checkWatchListConditions(totalPrediction)
...
const generateAgentPredictions = (): AgentPrediction[] => {
  ...
  agents.slice(0, 50).forEach((agent) => {
    ...
    const errorNames = ['공감표현 누락', '상담유형 오설정', '가이드 미준수', '본인확인 누락']
      .sort(() => Math.random() - 0.5)
      .slice(0, Math.floor(Math.random() * 3) + 1)
    const mainErrors = errorNames.map(name => ({
      name,
      count: Math.floor(Math.random() * 5) + 1,
      rate: Number((Math.random() * 5).toFixed(2))
    }))
