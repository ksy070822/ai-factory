...
│       ▼                                                             │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Stage 1: 이미지 전처리 및 품질 검증                            │   │
│  │  • 해상도 검증 (최소 640×480)                                 │   │
│  │  • 파일 크기 제한 (프로필 5MB, 증상 10MB)                      │   │
│  │  • MIME 타입 검증 (JPEG, PNG, HEIC)                          │   │
│  │  • Base64 인코딩 및 데이터 URI 변환                           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│       │                                                             │
│       ▼                                                             │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Stage 2: GPT-4o Vision 시각적 증상 분석                       │   │
│  │  • 6가지 카테고리 구조화 분석                                  │   │
│  │    - 외상 (wounds, lacerations, abrasions)                   │   │
│  │    - 부종 (swelling, inflammation)                           │   │
│  │    - 피부 이상 (redness, rash, lesions, hair loss)           │   │
│  │    - 안구 이상 (discharge, cloudiness, redness)              │   │
│  │    - 자세 이상 (limping, hunched posture)                    │   │
│  │    - 고통 신호 (facial expressions, body language)           │   │
│  │  • 각 항목별 심각도 스코어링 (1-5)                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│       │                                                             │
│       ▼                                                             │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Stage 3: Medical Agent 컨텍스트 통합                          │   │
│  │  • 시각적 소견을 텍스트 증상과 결합                            │   │
│  │  • 이미지 분석 결과를 감별진단 근거로 활용                      │   │
│  │  • 사진 기반 추가 질문 생성                                    │   │
│  └─────────────────────────────────────────────────────────────┘   │
│       │                                                             │
│       ▼                                                             │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ Stage 4: 병원 패킷 이미지 메타데이터 생성                       │   │
│  │  • 첨부 이미지 수 기록                                         │   │
│  │  • 시각적 소견 요약 (visual_findings)                          │   │
│  │  • 수의사용 이미지 분석 리포트 생성                             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
#### 2. GPT-4o Vision 멀티모달 분석
Information Agent에서 GPT-4o Vision을 호출하여 이미지 기반 증상 분석을 수행한다. 최대 3개 이미지를 동시에 분석하며, 구조화된 JSON 형식으로 결과를 반환한다.
...
#### 4. 병원 패킷 이미지 메타데이터
Ops Agent가 생성하는 병원 사전 패킷에 이미지 분석 결과가 포함된다. 수의사가 내원 전 시각적 소견을 사전에 확인할 수 있다.

```javascript
// hospitalPacket.js - 이미지 메타데이터 구조
{
  "hospital_packet": {
    "images_count": 2,
    "images": ["base64_image_1", "base64_image_2"],
    "visual_findings": {
      "summary": "좌측 귀 내부에 발적 및 삼출물 관찰",
      "detected_abnormalities": ["발적", "부종", "삼출물"],
      "severity_indicators": {
        "inflammation_level": "moderate",
        "pain_indicators": ["귀 긁는 행동", "머리 기울임"]
      }
    }
  }
}
...
```json
{
  "visual_analysis": {
    "image_quality": "good",
    "detected_body_part": "left_ear",
    "abnormalities": [
      {
        "type": "inflammation",
        "location": "inner_ear",
        "severity": 3,
        "description": "귀 내부에 발적과 부종이 관찰됨"
      }
    ],
    "pain_indicators": ["head_tilting", "scratching"],
    "recommended_actions": ["이경 검사", "세균 배양 검사"],
