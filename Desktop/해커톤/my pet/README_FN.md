...
반려동물 외상 및 질병 이미지를 분석하기 위한 3단계 이미지 처리 파이프라인을 구현하였다.
...
│  │  • 이미지에서 시각적으로 관찰 가능한 증상 분석                  │   │
│  │  • 텍스트 증상과 이미지를 함께 전달                            │   │
│  │  • visual_findings 필드로 소견 반환                           │   │
│  │  • Medical Agent 진단에 컨텍스트로 제공                        │   │
│  └─────────────────────────────────────────────────────────────┘   │
│       │                                                             │
│       ▼                                                             │
│  ┌─────────────────────────────────────────────────────────────┐   │
...
Information Agent에서 GPT-4o Vision을 호출하여 이미지 기반 증상 분석을 수행한다. 최대 3개 이미지를 동시에 분석하며, 시각적 소견을 텍스트로 반환한다.
...
```json
{
  "symptom_keywords": ["귀 문제", "가려움"],
  "body_part_focus": ["귀"],
  "severity_hint": "medium",
  "possible_categories": ["귀질환"],
  "owner_responses_summary": "3일 전부터 귀를 자주 긁고 냄새가 남",
  "notes_for_medical_agent": "외이염 가능성 높음. 이경 검사 권장",
  "visual_findings": "이미지에서 좌측 귀 내부에 발적과 삼출물이 관찰됨"
}
```
